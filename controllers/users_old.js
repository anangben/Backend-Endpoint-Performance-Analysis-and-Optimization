...
// User registration for old code
export const registerUser_old = async (req, res) => {
  console.time("registerUser_old_total");
  ...
  console.time("DB: check existing user old");
  const existingUser = await UserModel.findOne({ email: value.email });
  console.timeEnd("DB: check existing user old");
  ...
  console.time("CPU: hash password old");
  const hashedPassword = await bcrypt.hash(value.password, 12);
  console.timeEnd("CPU: hash password old");

  // Create user first (without token)
  console.time("DB: create user old");
  let user = await UserModel.create({
    ...value,
    password: hashedPassword,
    verificationToken: null,
  });
  console.timeEnd("DB: create user old");

  //Generate token and update user in a second DB write
  console.time("CPU + DB: token generation & save old");
  const verificationToken = crypto.randomBytes(16).toString("hex");
  user.verificationToken = verificationToken;
  await user.save();
  console.timeEnd("CPU + DB: token generation & save old");

  // Send email verification to user via their email address
  console.time("I/O: send email old");
  await sendEmail({
...
`,
  ...
  console.timeEnd("I/O: send email old");

  console.timeEnd("registerUser_old_total");
  res.status(201).json({
    message:
